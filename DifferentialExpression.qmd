...
Bakta_tsv_file <- list.files(file.path(here(), "Data", "Bakta_annotated_files", paste0("Strain_",params$strain)), full.names = T,pattern=".tsv")
...
strain_colors <- list(
  '93' = c("WT_0" = "#b3b0aa", "WT_125" = "#74726a", "CHX_Adapted_0" = "#BFFCC6", "CHX_Adapted_125" = "#228B22"),
  '73' = c("WT_0" = "#b3b0aa", "WT_125" = "#74726a", "CHX_Adapted_0" = "#FFCCE6", "CHX_Adapted_125" = "#cd3363"),
  '78' = c("WT_0" = "#b3b0aa", "WT_125" = "#74726a", "CHX_Adapted_0" = "#B3D7FF", "CHX_Adapted_125" = "#4D85BD")
)
  ...
  sapply(selectedGenes, function(genetype) {
    ...
    sapply(locus, function(x) {
      ...
      p <- ggplot(Counts, aes(x = CHX, y = count, shape = Genotype, color = Genotype_CHX, fill = Genotype_CHX)) +
        ...
        guides(
          fill = guide_legend(override.aes = list(size = 5, linetype = c(0, 0, 0, 0), shape = c(16, 16, 17, 17), color = c(Genotype_CHX.colour), fill = c(NA, NA, NA, NA))),
...
txtogene <- readr:::read_tsv(file.path(params$nfcore_output_Path,"star_salmon", "tx2gene.tsv"), col_names = F)
...
# Plot PCA
plot_pca <- autoplot(PCA, data = metadata, colour = "Genotype_CHX", shape = "Genotype", size = 5) +
  theme_bw() +
  theme(
    legend.position = "top",
  ...
  guides(
    colour = guide_legend(order = 1, override.aes = list(size = 3, linetype = c(0, 0, 0, 0), shape = c(17, 17, 16, 16))),
    shape = "none"
  ) +
...
```{r DEseq CHX125.vs.Untreated_inCHX_Adaptedcells}
res_CHXresponse <- results(dds, list(c("CHX_125_vs_0", "GenotypeCHX_Adapted.CHX125")))
res_CHXresponse <- lfcShrink(dds, coef = list(c("CHX_125_vs_0", "GenotypeCHX_Adapted.CHX125")), type = "ashr", res = res_CHXresponse)

fdr <- processDeseq(res_CHXresponse, resultsPath = resultsPath, comparison = "CHX125.vs.Untreated_inCHX_Adaptedcells", GeneInfos = GeneInfos, by = c("gene", "Locus Tag"))
```

### Volcano Overview

```{r Plot Vulcano CHX125.vs.Untreated_inCHX_Adaptedcells}
De_all_CHXresponse <- list.files(file.path(resultsPath, "CHX125.vs.Untreated_inCHX_Adaptedcells"), pattern = "_Differential_Expression_all.xlsx", full.names = T)
...
```{r DEtable CHX125.vs.Untreated_inCHX_Adaptedcells}
if (nrow(fdr) == 0) {
...
```{r GSEA CHX125.vs.Untreated_inCHX_Adaptedcells}
...
```{r DEseq CHX_Adapted.vs.WT_inCHX0Conditions}

res_MutatationEffect_CHX0 <- lfcShrink(dds, contrast = c("Genotype", "CHX_Adapted", "WT"), type = "ashr")
fdr <- processDeseq(res_MutatationEffect_CHX0, resultsPath = resultsPath, comparison = "CHX_Adapted.vs.WT_inCHX0Conditions", GeneInfos = GeneInfos, by = c("gene", "Locus Tag"))
```

### Volcano Overview

```{r Plot Vulcano CHX_Adapted.vs.WT_inCHX0Conditions}
De_all_MutatationEffect_CHX0 <- list.files(file.path(resultsPath, "CHX_Adapted.vs.WT_inCHX0Conditions"), pattern = "_Differential_Expression_all.xlsx", full.names = T)
...
```{r DEtable CHX_Adapted.vs.WT_inCHX0Conditions}
if (nrow(fdr) == 0) {
  print("No significant genes found")
} else {
  DT::datatable(fdr[, c("gene", "gene_id", "baseMean", "log2FoldChange", "pvalue", "padj", "COG", "COG_category", "GO", "comparison")],
...
```{r GSEA CHX_Adapted.vs.WT_inCHX0Conditions}
...
```{r DEseq CHX_Adapted.vs.WT_inCHX125Conditions}
res_MutatationEffect_CHX125 <- results(dds, list(c("Genotype_CHX_Adapted_vs_WT", "GenotypeCHX_Adapted.CHX125")))
res_MutatationEffect_CHX125 <- lfcShrink(dds, list(c("Genotype_CHX_Adapted_vs_WT", "GenotypeCHX_Adapted.CHX125")), res = res_MutatationEffect_CHX125, type = "ashr")
fdr <- processDeseq(res_MutatationEffect_CHX125, resultsPath = resultsPath, comparison = "CHX_Adapted.vs.WT_inCHX125Conditions", GeneInfos = GeneInfos, by = c("gene", "Locus Tag"))
```

### Volcano Overview

```{r Plot Vulcano CHX_Adapted.vs.WT_inCHX125Conditions}
De_all_MutatationEffect_CHX125 <- list.files(file.path(resultsPath, "CHX_Adapted.vs.WT_inCHX125Conditions"), pattern = "_Differential_Expression_all.xlsx", full.names = T)
...
```{r DEtable CHX_Adapted.vs.WT_inCHX125Conditions}
if (nrow(fdr) == 0) {
  print("No significant genes found")
} else {
  DT::datatable(fdr[, c("gene", "gene_id", "baseMean", "log2FoldChange", "pvalue", "padj", "COG", "COG_category", "GO", "comparison")],
...
```{r GSEA CHX_Adapted.vs.WT_inCHX125Conditions}
...
col_annot <- ComplexHeatmap::HeatmapAnnotation(df = metadata[, c("Genotype", "CHX")], show_legend = TRUE)
...
ht <- ComplexHeatmap::Heatmap(
  ...
  column_split = factor(c(rep("WT", 6), rep("CHX adapted", 6)), levels = c("WT", "CHX adapted")), 
...
selected_genes <- plotNormalizedValues(De_all_MutatationEffect_CHX0, res_MutatationEffect_CHX0, 
                                       ...
                                       comparison = "CHX_Adapted.vs.WT_inCHX0Conditions", 
...
supp.labs <- c("activated in CHX adapted", "activated in WT")
names(supp.labs) <- c("activated", "suppressed")
GSE <- dotplot(gse_MutatationEffect_CHX0, showCategory = 5, split = ".sign") + coord_flip() + facet_grid(.sign ~ ., labeller = labeller(.rows = supp.labs)) + theme(legend.box = "horizontal", axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
...
left_side <- plot_grid(plot_pca, Volcano_MutatationEffect_CHX0 + theme(legend.position = "bottom", legend.text = element_text(size = 10)) + guides(color = guide_legend(override.aes = list(size = 1.5))), labels = c("A", "B"), label_size = 12, ncol = 1, rel_heights = c(1, 1), scale = c(0.98)) +
  theme(plot.background = element_rect(fill = "#FFFFFF", colour = NA))
final_sample <- plot_grid(left_side, right_side, rel_widths = c(1, 1.5), label_size = 12, ncol = 2) +
  theme(plot.background = element_rect(fill = "#FFFFFF", colour = NA))
final_sample <- plot_grid(final_sample, GSE, ncol = 1, rel_heights = c(3, 1.5), scale = c(1, 0.98), labels = c("", "E")) +
  theme(plot.background = element_rect(fill = "#FFFFFF", colour = NA))

ggsave(final_sample, file = file.path(resultsPath, paste0("final_sample_GSE.svg")), width = 15, height = 20)
final_sample
```
