...
theme_custom <- function() {
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1,size = 7,lineheight = 0.9,face = "plain"),
    axis.text.y = element_text(size = 7,lineheight = 0.9,face = "plain"),
    legend.text = element_text(size = 7,lineheight = 0.9),
    legend.title = element_text(size = 7,lineheight = 0.9,face = "bold"),
    axis.title = element_text(size = 9,lineheight = 0.9,face = "plain"),
    plot.title = element_text(size = 7,lineheight = 0.9,face = "plain"),
...
MyVolcano <- function(x, removePattern = "_.*") {
  ...
  #   xpos = c(-Inf, Inf),
  #   ypos = c(Inf, Inf),
  #   annotateText = c(paste0("Higher in: ", comparisons[2]), paste0("Higher in: ", comparisons[1])),
  #   hjustvar = c(-0.1, 1),
  #   vjustvar = c(1, 1)
  # )
  ...
  sapply(selectedGenes, function(genetype) {
    ...
    sapply(locus, function(x) {
      ...
      p <- ggplot(Counts, aes(x = CHX, y = count, shape = Genotype, color = Genotype_CHX, fill = Genotype_CHX)) +
        ...
        guides(
          fill = guide_legend(order = 1,title.position = "top", title.hjust = 0,legend.title.align=0,override.aes = list(size = 3, linetype = c(0, 0, 0, 0), shape = c(17, 17,16, 16), color = c(Genotype_CHX.colour[order(names(Genotype_CHX.colour))]), fill = c(NA, NA, NA, NA))),
...
right_side <- plot_grid(heatmap, expression, legend_boxplots, labels = c("C", "D"), label_size = 10, ncol = 1, rel_heights = c(1.3, 1.2, 0.1), scale = c(0.98)) 
...
ggsave(final_sample, file = file.path(resultsPath, paste0("final_sample_GSE.tif")), dpi = 600, width = 10, height = 11)
final_sample
```
...
if (params$strain == 93) {
  ...
  ggsave(CHX_effect_93, file = file.path(resultsPath, paste0("final_sample_GSE_OnlyTreatmenteffect.tif")), dpi = 600, width = 13, height = 6)
