...
# # Filter files for specific comparison: CHX_Adapted vs WT in CHX condition
relevant <- grep("CHX_Adapted.vs.WT_inCHX", relevant_fdr, value = TRUE)
```
...
upPlot <- ComplexHeatmap::UpSet(m,
  ...
  top_annotation = upset_top_annotation(m, height = unit(7, "cm")),
...
radarplots <- lapply(c("S. salivarius", "S. vestibularis", "S. mitis"), function(x) {
  ...
  radar_regulated <- ggradardata[ggradardata$Regulation == "Upregulated", -1] %>%
    ggradar(
      grid.min = 0, grid.mid = max(ggradardata[, -c(1:2)]) / 2, grid.max = max(ggradardata[, -c(1:2)]),
      label.gridline.max = "25%", gridline.mid.colour = "black",
      label.gridline.min = "0%", label.gridline.mid = "10%", background.circle.colour = "#1A120B",
      group.point.size = 4, group.line.width = 0.75, group.colours = group_colours,
      fill = TRUE, fill.alpha = 0.3, legend.position = "right"
    )

  # Extract and arrange legend with plot
  legend <- get_legend(radar_regulated)
  plot_radar_list <- plot_grid(radar_regulated + theme(legend.position = "none"), legend, ncol = 2)
...
legend <- ggtexttable(sidebyside,
  ...
  table_cell_font(row = c(1:nrow(sidebyside)), column = c(1, 3), face = "bold") %>%
  tab_add_vline(at.column = c(2, 4), column.side = "left", from.row = 1, linetype = 2)
...
plot_radar
# Generate a final plot combining the UpSet plot and radar plot

# ggsave(plot = plot, file.path(ComparisonPath, paste0("common.svg")), width = 48, height = 30, units = "cm")
# save_plot(file.path(ComparisonPath, paste0("common.png")), plot, base_width = 48, base_height = 30, units = "cm")
```
...
relevant <- grep("CHX_Adapted.vs.WT_inCHX0", relevant_all, value = TRUE)
...
antibioticsPlot <- ggplot(antibiotica) +
  ...
  geom_point(aes(x = gene_id, y = comparison, color = meanlog2FoldChange, shape = drug_class), size = 15) +
  guides(
    color = guide_legend(title = bquote(~ Log[2] ~ "fold change"), override.aes = list(size = 10)),
    fill = "none", shape = guide_legend(title = "Drug class", override.aes = list(size = 10))
  ) +
...
regulated Antibiotic Resistance Genes",
    subtitle = "CHX Adapted vs WT without CHX exposure",
...
plot_upper <- plot_grid(grid::grid.grabExpr(draw(upPlot)), plot_radar, rel_widths = c(1.2, 1), labels = c("A", "B"), label_size = 15, ncol = 2, scale = 0.95)
plot_final <- plot_grid(plot_upper, antibioticsPlot, rel_heights = c(3, 1), labels = c("", "C"), label_size = 15, ncol = 1, scale = c(1, 0.95)) +
  theme(plot.background = element_rect(fill = "#FFFFFF", colour = NA))

# Save the final plot
ggsave(plot = plot_final, file.path(ComparisonPath, paste0("common_antibiotics.svg")), width = 50, height = 35, units = "cm")
plot_final
```
